...
class Akun:                                                                         
    ...
    def deposit(self, nominal):
        ...
        self.saldo += nominal
        if self.saldo>self.limit_saldo:
            keuntungan_bank += self.saldo-self.limit_saldo
            self.saldo -= self.saldo-self.limit_saldo
        return self.saldo
        pass
    ...
    def hutang(self, tujuan, nominal, bunga):
        ...
        elif self.tipe_tabungan == "riajuu":
            kembalian = nominal + (0.15*nominal)
        elif self.tipe_tabungan == "normies":
...
class Weaboo(Akun):
    ...
    def hutang(self, penerima, nominal):
        super().hutang(tujuan, nominal, "weaboo")
...
class Riajuu(Akun):
    ...
    def hutang(self, penerima, nominal):
        super().hutang(tujuan, nominal, "riajuu")
...
class Normies(Akun):
    ...
    def hutang(self, penerima, nominal):
        super().hutang(tujuan, nominal, "normies")
...
def hutang_action(): #Ini adalah fungsi yang akan dipanggil saat button untuk hutang ditekan
    ...
    orang2 = nasabah[orang2]
    nomnom = nominal_entry.get()
    pesan['text'] = nasabah[orang1].hutang(orang2, nomnom)
    # TODO : Lakukan implementasi untuk mengambil data dari form, lalu memanggil fungsi untuk melakukan hutang, lalu mengganti label
