...
class PopUpWindow(object):
    ...
    def __init__(self, master):
        self.top=top=tk.Toplevel(master, width=500, height=100)
        top.title("Set A Time")
        self.frame1 = tk.Frame(top, width=20, height=1)
        self.frame1.pack(padx=50)        

        self.info1 = tk.Label(self.frame1, text="Please set a time")
        self.info1.grid(row=0, column=0, columnspan=4, sticky=tk.N)

        #Entries
        self.entryhour = tk.Entry(self.frame1, width=4)
        self.spacer1 = tk.Label(self.frame1, text=":")
        self.entryminute = tk.Entry(self.frame1, width=4)
        self.spacer2 = tk.Label(self.frame1, text=":")
        self.entrysecond = tk.Entry(self.frame1, width=4)
        self.confbutton = tk.Button(self.frame1, text="Confirm", command=self.confirm)
...
class MainWindow(object):
    ...
    def popup(self):
        ...
        try:
            ...
            self.buttonsettimer["state"] = "normal"
            messagebox.showerror("Error : Input", "Please input hour ( 0-23 ), minute ( 0-59 ), \nand second ( 0-59 ) accordingly")
            self.alarm_is_set = 0
    ...
    def addalarmui(self):
        if self.labelalarm_is_set == 0:
            self.labelalarm_is_set = 1
        ...
        elif self.labelalarm_is_set == 1:
            self.labelalarm['text'] = "Alarm set at {}:{}:{}".format(hour,minute,second)
    ...
    def reset(self):
        self.buttonsettimer["state"] = "normal" 
        global alarmhms, hour, minute, second
        if self.alarm_is_set == 1:
